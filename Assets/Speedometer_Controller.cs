...
public class Speedometer_Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        //Show speed in KPH
        if(Speed_In_KPH){
            speed = Car_Rb.linearVelocity.magnitude * 3.6f;
        }

        //Show speed in MPH
        if(!Speed_In_KPH){
            speed = Car_Rb.linearVelocity.magnitude * 2.237f;
        }
    ...
    private void FixedUpdate(){
        ...
        if(!is_braking && !is_reversing){   
            ...
            if(Gear_Speeds.Last() <= speed){
                Car_Rb.linearVelocity = Vector3.ClampMagnitude(Car_Rb.linearVelocity, Maximum_Speed);
            }
